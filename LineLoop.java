...
//System.out.println("Λ = "+Lamda);
  counter.addA("E131",1);
  //retrieve v
  v=facet_totals[facet1][1];
  //compute lamda_dash
  lamda_dash=(r1_r2-L*Lamda)/2;
//System.out.println("λ' = "+lamda_dash);
 ...
 // System.out.println("Λ stored = "+Edge[adjacent_edge][21]);
...
else
 {
   for(int euler=0;euler<Edge.length/2;euler++)
      if(((int)Edge[euler][18]==facet2)&&(((int)Edge[euler][19]==facet1)))
       ...
       // System.out.println("Λ retrieved = "+Lamda);
       // Lamda=-Lamda;
        //System.out.println("Λ = "+Lamda);
...
Cij += v*facet_totals[i][0]/2;
 ...
 Xaxis=vector.magnitude(data.getObs(obs))/L;
 Xaxis=Math.log10(Xaxis);
 Cij=Math.log10(Math.abs((Cij-e)/e));
 
 


System.out.print("X Point :"+(obs+1)+"  = "+Xaxis);
System.out.println("                       Y point :"+(obs+1)+"  = "+Cij);
}// end obs loop
//counter.PrintVEF();
