...
public double deltaLamdaStar(double L,vector ric,double DeltaStarBar)
  ...
  result=result+Math.pow(Delta1(rc,rj2,Rc,Rj2),2)/2;
  ...
  public void Extrinsic_Loop()
{
    int adjacent_edge=0;
     ...
     double anomaly=0.0; double anomaly1=0.0; 
...
for (int edge_count=0;edge_count<data.getEdges()/2;edge_count++)
{
 ...
 //implementation Barcelona (4) for bij
  ...
  ni_curl1=vector.mulScalar(ni1, math.signum(v1));
   ...
   //vh2ArctanL=vector.addVec(vh2ArctanL, vh2ArctanL_1);
   //n term
  ...
  //  System.out.println("r1,r2  for obs :"+(obs+1)+"  = "+r_bar);
   ...
   ric=vector.subVec(data.getObs(obs),Ric);
    // System.out.println("Total anomaly for volume for obs :"+(obs+1)+"  = "+Robs2_bar);
   rp=vector.subVec(data.getObs(obs),Rp);
   ...
   //System.out.println("Total anomaly for volume for obs :"+(obs+1)+"  = "+rc_curl);
   lamda_vol=(h1*Lambda)/r_curl;
   // System.out.println("Total anomaly for volume for obs :"+(obs+1)+"  = "+lamda_vol);
  ...
  volume_edge_anomaly=delta_b_star_volume;
  vector_Area=data.getArea(facet);Area=vector.magnitude(vector_Area);
 // System.out.println("Total anomaly for volume for obs :"+(obs+1)+"  = "+Area);
  volume_edge_anomaly=vector.addVec(volume_edge_anomaly, vector.mulScalar(ricrp(ric,rp,Ric, Rp),Area));
  anomaly1=vector.dot(volume_edge_anomaly, r_bar);
  // System.out.println("Total volume facet2 for obs :"+(obs+1)+"  = "+anomaly);
   facet_totals_volume[facet][0]+=anomaly1;
   
   //System.out.println("Total anomaly for volume for obs :"+(obs+1)+"  = "+Rp);
}// end edge loop
...
{double x,y,z;
v=facet_totals[i][1];
double v_volume=facet_totals_volume[i][1];

Cij += 
        v*facet_totals[i][0];
Cij_volume+=facet_totals_volume[i][0];
...
System.out.println("Total anomaly  for obs :"+(obs+1)+"  = "+Cij);
...
{
    volumeUndirected m = new volumeUndirected();
    m.Facet_Loop();
    m.Extrinsic_Loop();
  } 
