...
package loop;
public class VertexNewLoopStructure1 {
  Data data=new Data();
  int target=2;
  int facetA,facetB,facetC,vertexA,vertexB,vertexC,edgeA,edgeB,edgeC;
  double[][] Edge=data.getEdge_Struct();VertexNewLoopStructure1() {}
...
{int adjacent_edge=0;
     ...
     double r1_l1 = 0,r2_l2 = 0,r1l1,r2l2;
...
for (int edge_count=0;edge_count<data.getEdges()/2;edge_count++)
{
  ...
  //counter.addA("E13",6);counter.addB("E13",4);
...
//{//sign check
   ...
   if (l2 <= 0) //(we know l2 >l1)
    ...
    {   System.out.println("branch a :"+edge_count);
        ...
        //b_h=math.signum(l1 + l2)  *Math.log( (Robs2_len + Math.abs(l2)) / (Robs1_len + Math.abs(l1)));
        //b_h=-Math.log(Robs1_len-l1/Robs2_len-l2);
...
//(Math.atan2( (l2 * Math.abs(v)), (h * Robs2_len)) -Math.atan2( (l1 * Math.abs(v)), (h * Robs1_len))));
        b_n1=-Math.atan2(r2_l2+v1, h1)+Math.atan2(r1_l1+v1, h1);
          //b_n            = ( (Math.atan2(l2, h1)) - Math.atan2(l1, h1) -
//(Math.atan2( (l2 * Math.abs(v1)), (h * Robs2_len)) -Math.atan2( (l1 * Math.abs(v1)), (h1 * Robs1_len))));
    ...
    if (l1<0)
        ...
        b_h=Math.log((r2l2)/(r1l1));
          //b_h =  Math.log( (Robs1_len + Math.abs(l1)) * (Robs2_len + Math.abs(l2)) / vector.dot(vector_r0, vector_r0));
        counter.addA("E13",1);counter.addC("E13",1);
...
//Cij += v*(sum_hC[i]);
//Cij +=Math.abs(v)*sum_half_Omega_bar[i];
...
{
    VertexNewLoopStructure1 m = new VertexNewLoopStructure1();
    m.Facet_Loop();
    m.Extrinsic_Loop();
